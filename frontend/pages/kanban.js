const KanbanPage = {
    ...
    try {
      ...
      if (sel) {
        sel.innerHTML = '<option value="">담당자 없음</option>' +
          this.members.map(m => `<option value="${m.id}">${m.email} (${m.is_owner ? 'owner' : 'member'})</option>`).join('');
      }
  ...
  _openDetail(id) {
    ...
    const memberOptions = '<option value="">담당자 없음</option>' +
      this.members.map(m => `<option value="${m.id}" ${m.id === task.assignee_id ? 'selected' : ''}>${m.email} (${m.is_owner ? 'owner' : 'member'})</option>`).join('');
